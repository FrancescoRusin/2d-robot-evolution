...
public class Mappers {
  ...
  @Alias(
      name = "dsToNIV", value = // spotless:off
                    """
                                noisedDsToNIV(
                                    bodySizeSigma = 0;
  ...
  public static <X> InvertibleMapper<X, Supplier<NumIndependentVoxel>> noisedDsToNIV(
    ...
    return beforeM.andThen(
        InvertibleMapper.from(
            (supplier, values) -> () -> {
              final double sideLength = Math.max(
                  0.0001,
                ...
                if (sensorType instanceof SenseDistanceToBody sd) {
                  newSensors.add(
                      Sensors.d(
                          ...
                          Math.max(0.0001, sd.distanceRange() * randomGenerator.nextGaussian(1, sensorNoise))
                      )
                  );
                } else {
                  newSensors.add(s);
              ...
              final double sideContraction = new DoubleRange(0.0001, .4)
                  .clip(Voxel.Material.DEFAULT_AREA_RATIO * randomGenerator.nextGaussian(1, contractionNoise));
