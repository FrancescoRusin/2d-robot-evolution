...
public class NoisedVideoMaker {
  ...
  public static void noisePilingVideos(String exp) throws IOException {
    ...
    while (Objects.nonNull(line = reader.readLine())) {
      ...
      final String key = Arrays.stream(splitLine)
          .toList()
          .subList(firstParamIndex, firstParamIndex + 4)
          .stream()
    ...
    TaskVideoBuilder<Supplier<EmbodiedAgent>> taskVideoBuilder = (TaskVideoBuilder<Supplier<EmbodiedAgent>>) nb.build(
        "sim.taskVideoBuilder(task = s.task.standPiling(duration = 20; nOfAgents = 8))"
    );
